...
5. ~~Begin Phase 4d (individually tracked inventory) or Phase 4e (simple linked returns)~~ — 4d done on `phase/p4-point-of-sale`: `inventory_units` schema (generated `27` identifiers, `available`/`reserved`/`sold` statuses plus inert `inspection`/`damaged`/`discarded`/`rtv`/`in_transfer` values, exact acquisition cost), `Inventory::CreateInventoryUnit` bootstrap service (enforces `inventory.unit.manage`), `Inventory::Reserve`/`ReleaseReservation`/`ConvertReservation` extended to lock and transition the exact unit, `Pos::AddLine`/`UpdateLineQty`/`ResolveScan` extended for unit-backed lines, the `inventory.unit.manage` permission with a minimal admin UI, and concurrency/completion tests. See [phase-04-point-of-sale.md](phases/phase-04-point-of-sale.md) 4d exit criteria (including the open follow-up on the `acquisition_source_type`/status enum getting ahead of designed workflows) and [service-catalog.md](service-catalog.md). Phase 4e (simple linked returns) remains open.
