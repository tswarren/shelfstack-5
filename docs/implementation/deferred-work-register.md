...
| DWR-010 | 6.5e — Keyboard and scanner stabilization | 6.5 | delivery_debt | [phase-06.5-cashier-workspace.md](phases/phase-06.5-cashier-workspace.md); absorb into DWR-067 Gate E when Phase 11 is scheduled | None (or Phase 11 Gate E) | unscheduled | [#51](https://github.com/tswarren/shelfstack-5/issues/51) |
...
| DWR-017 | Customer receipt product (templates, gift receipts, ESC/POS / printer queues) | 7 park | delivery_debt | [phase-7-optional-receipt-printing.md](../temp_draft/phase-7-optional-receipt-printing.md) (non-governing); stage HTML/gift/SV slip into DWR-067 Gate D when Phase 11 is scheduled; ESC/POS fleets remain later | Explicit product design; not Phase 7 core | later_extensions | — |
...
| DWR-020 | Searchable record-picker / nested combobox (shared linking UX) | 4f | delivery_debt | Gate 8a foundation merged to `main` (PR [#102](https://github.com/tswarren/shelfstack-5/pull/102), closes [#95](https://github.com/tswarren/shelfstack-5/issues/95)); Creator type added in Gate 8b Slice 1; design [interaction-patterns.md](../design/interaction-patterns.md) | Keep row open for PO/receipt nested line adoption | unscheduled | [#95](https://github.com/tswarren/shelfstack-5/issues/95) (foundation, closed); follow-on TBD |
...
| DWR-028 | Product import `return_to`: reject malformed percent-encoding before post-commit redirect | 8c | delivery_debt | Resolved — `sanitize_return_to` parses nested query; `return_path` rescues `ArgumentError`; controller unit coverage. Issue [#116](https://github.com/tswarren/shelfstack-5/issues/116) | — | — | [#116](https://github.com/tswarren/shelfstack-5/issues/116) |
...
| DWR-066 | Product show rail: build stock summary when any of stock/receipt/PO/cost caps is present | 10 | delivery_debt | Phase 10 MVP review — `BuildProductSummary#build_stock` returns nil without `stock_view`, so independently authorized last-received / on-order / cost rows are underexposed (not a leak). MVP roles still bundle these with `stock_view`. **Must revisit before** any role grants receipt visibility, purchasing/on-order visibility, or cost visibility without `stock_view`. | Return a stock summary object when any relevant capability exists; nil unauthorized fields; rail gates each row | unscheduled | — |
| DWR-067 | POS shell and workspace revamp (candidate Phase 11) | 6.5+ | delivery_debt | [phase-11 sequencing](../temp_draft/phase-11/00-post-phase-10-sequencing.md); [phase stub](../temp_draft/phase-11/01-phase-11-pos-shell-revamp-stub.md); workspace drafts under [pos_revamp/](../temp_draft/pos_revamp/) (non-governing until promoted) | Phase 10 fully complete; promote stub to `phases/phase-11-…`; absorb DWR-010; stage DWR-017 Gate D | unscheduled | — |
