...
| DWR-017 | Customer receipt product (templates, gift receipts, ESC/POS / printer queues) | 7 park / 11 / 11.1 | delivery_debt | Gate 11D Must delivered on [#150](https://github.com/tswarren/shelfstack-5/pull/150) / phase-11 §15.1. Phase 11 closed. [phase-11…](phases/phase-11-pos-shell-and-workspace-revamp.md); v1 build contract [../temp_draft/phase-11.1/00_pos_printing-simplified.md](../temp_draft/phase-11.1/00_pos_printing-simplified.md) | **Delivered Must (Phase 11):** read-only Receipt (`pos/receipt/*`), browser-printable HTML, snapshot line description/identifier, store header/footer+address, lookup, reprint/`REPRINT` + `pos.receipt.reprint`, non-mutating lookup, print/display does not reverse completion. **Delivered Should (partial):** linked return from Receipt Lookup; return/mixed/post-void document banners; discount/net merchandise rows. **Should residual → Phase 11.1 draft:** gift receipt + `gift_receipt_footer`; Code 128 barcode; server-owned reprint routes; masked customer; Post-Void print polish; fuller templates; INV-POS-014 print-event audit. **Header/footer admin UI:** DWR-019. **Later:** ESC/POS fleets / queues | 11.1 (draft); later_extensions (hardware) | [#134](https://github.com/tswarren/shelfstack-5/issues/134) (Gate D Must closed with Phase 11) |
...
Phase 11 (closed):             DWR-067 resolved; DWR-017 Should → Phase 11.1 draft
