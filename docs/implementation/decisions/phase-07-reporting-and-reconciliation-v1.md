...
- **MVP delivery note:** finalization is fully immutable (header + children); append-only superseding and linked domain-correction resolutions are deferred ([#90](https://github.com/tswarren/shelfstack-5/issues/90), [#89](https://github.com/tswarren/shelfstack-5/issues/89)). Operable resolutions are Explain / Accept variance / accept unavailable, matched to comparison state.
