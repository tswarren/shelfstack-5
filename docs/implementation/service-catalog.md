...
| `Pos::ResolveCardRefundTenderReconciliation` | Point of Sale | 6d | Yes | No | Txn + preparation + tender | Preparation/tender, outcome (`externally_voided` / `validated_and_accepted` / `replaced`), reason | Clears recon or voids tender; no generic clear |
| `Pos::ResolveCardRefundOrphan` | Point of Sale | 6d | Yes | No | Preparation | Orphan prep, resolution kind, reason, refs | Resolves `recorded_orphan` with typed outcome + audit |
