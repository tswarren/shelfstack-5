...
| `Catalog::Providers::ParseProviderDate` | Catalog and Products | 8b | No | Yes | None | Provider date string | `[date, precision]` on the `Catalog::PartialPublicationDate` contract, or `[nil, nil]` for blank/unrecognized text (never guesses) |
...
- Gate 8c (create-from-ISBN): `FindExistingProduct` / `PreviewProductImport` / `ProductImportPreviewToken` / `CreateFromEnrichment`; permission `catalog.create_from_enrichment` (persist still also needs `catalog.product.create`). Preview never writes; preview issues a signed token; accept verifies the token and builds enrichment-event provenance only from the verified payload (operator-editable bibliographic fields come from the form). Create writes one enrichment event in the same transaction. Operator UI extends `ProductImportsController` (`POST /product_imports/preview`, `POST /product_imports/accept`) with Catalog nav “Create from ISBN”.
