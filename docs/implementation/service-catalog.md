...
| `Pos::PostVoidTransaction` | Point of Sale | 6a | Yes | Yes (completion idempotency key) | Session, original txn/lines/tenders, inventory, SV accounts, Product Requests (canonical order); requires recorded `PosPostVoidCardPreparation` per card tender | Original completed txn, session, actor, reason, approver | New completed reversing txn; consumes recorded post-void card preparations; inventory via `Inventory::ReverseLedgerEntry`; fulfilment via `Requests::ReverseFulfillment`; SV via `StoredValue::PostEntry` (`reversal`) |
| `Pos::PreparePostVoidCardConfirmation` | Point of Sale | 6a | Yes | No | Original completed card tender | Original card tender, actor | Durable `PosPostVoidCardPreparation` (`prepared`) before terminal use |
| `Pos::RecordPostVoidCardConfirmation` | Point of Sale | 6a | Yes | Yes (same auth replay) | Preparation row | Preparation + terminal confirmation refs | Marks preparation `recorded`; survives later post-void failure |
