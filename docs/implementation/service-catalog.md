...
| `Pos::AddCardTender` | Point of Sale | 4c/6a | Yes | No | Transaction (`lock`); finalizes linked-return residuals before balance check | Transaction, card Tender Type, amount, refs, request UUID, actor | `authorized` when attachable; after refs validate, attach failure persists `void_required` under the same `recording_idempotency_key` |
