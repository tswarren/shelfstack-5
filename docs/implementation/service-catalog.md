...
| `Pos::AddUnlinkedReturnLine` | Point of Sale | 11B | Yes | No | Transaction (`lock`); `pos.return.create`; `no_receipt` also `pos.return.no_receipt` + `maximum_no_receipt_return_cents` (`AuthorizeAction` / `no_receipt_return`) | Open Transaction; source `external_receipt`/`gift_receipt`/`no_receipt`; reason; disposition; explicit `unit_price_cents`; product variant (quantity/none) **or** open-ring department/description; optional approver | Pending return line with explicit refund/tax basis; unlinked tax via `Tax::CalculateTransaction` direction `return`; cost snapshots `unknown`; rejects individually tracked variants; inventory via `PostCustomerReturn` on product complete |
