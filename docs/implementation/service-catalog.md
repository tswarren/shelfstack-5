...
| `Pos::OpenSession` | Point of Sale | 4a | Yes | No | Business Day (`lock`), then device/drawer open-session checks | Business Day, store, device, cashier, optional drawer | Open Session; day status rechecked under lock; device and cash-drawer exclusivity |
...
| `Pos::RecallTransaction` | Point of Sale | 4a | Yes | No | Session then Transaction (`lock`) | Transaction, session, actor | Reopened Transaction bound to the recalling Session; Session status rechecked under lock; concurrent recall has exactly one winner |
...
| `Pos::OverridePrice` | Point of Sale | 4b | Yes | No | Transaction then Line (`lock`) | Line, requested unit price, actor, optional reason/approver | Overridden `unit_price_cents`; audit event; triggers `Pos::RecalculateTransaction` |
