...
- Gate 8c (create-from-ISBN): `FindExistingProduct` / `PreviewProductImport` / `CreateFromEnrichment`; permission `catalog.create_from_enrichment` (persist still also needs `catalog.product.create`). Preview never writes; create writes one enrichment event in the same transaction. Operator UI extends `ProductImportsController` (`POST /product_imports/preview`, `POST /product_imports/accept`) with Catalog nav “Create from ISBN”.
