...
| `Pos::PreparePostVoid` | Point of Sale | 6a | Yes | No | Original completed txn; creates `PosPostVoidPreparation` (`approved`) with bound `pos.post_void.approve` / self-approve; eagerly prepares card children | Original txn, actor, reason, approver | Approved durable plan before any terminal card confirmation |
