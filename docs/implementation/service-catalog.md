...
| `Pos::PrepareCardRefund` | Point of Sale | 6d | Yes | No | Return txn + linked originals (`RefundLockOrder`) | Return txn, card tender type, amount, optional original / exception | Durable `PosCardRefundPreparation` (`prepared`); plan snapshot + fingerprint; owns exception approval; commercially locks txn |
