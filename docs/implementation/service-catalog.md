...
| `Pos::AddCardTender` | Point of Sale | 4c/6a | Yes | No | Transaction (`lock`); finalizes linked-return residuals before balance check | Transaction, card Tender Type, amount, authorization code, optional terminal reference, actor | `authorized` `PosTender`; amount exceeding finalized balance is retained with `requires_reconciliation` (external auth must not be discarded) |
...
| `Pos::PreparePostVoid` | Point of Sale | 6a | Yes | No | Original completed txn; blocks when unresolved post-void card orphans exist; creates `PosPostVoidPreparation` (`approved`) with bound approval; eagerly prepares card children | Original txn, actor, reason, approver | Approved durable plan before any terminal card confirmation |
| `Pos::ResolvePostVoidCardOrphan` | Point of Sale | 6a | Yes | No | Unresolved `recorded_orphan` post-void card prep | Orphan, kind (`external_void_confirmed` / `adopt_as_confirmation` / `accepted_financial_exception`), reason | Clears orphan block; adopt converts to consumable `recorded` under approved parent |
| `Pos::AbandonPostVoid` | Point of Sale | 6a | Yes | No | Approved parent with no recorded card children | Preparation, actor | Abandons plan and prepared card children |
| `Pos::PostVoidTransaction` | Point of Sale | 6a | Yes | Yes (completion idempotency key) | Session, original txn/lines/tenders, inventory, SV accounts, Product Requests (canonical order); requires approved `PosPostVoidPreparation` with matching commercial fingerprint; requires recorded `PosPostVoidCardPreparation` per card tender | Original completed txn, session, actor, completion key | New completed reversing txn; consumes parent preparation + approval and recorded card children; inventory via `Inventory::ReverseLedgerEntry`; fulfilment via `Requests::ReverseFulfillment`; SV via `StoredValue::PostEntry` (`reversal`) |
| `Pos::PreparePostVoidCardConfirmation` | Point of Sale | 6a | Yes | No | Approved parent plan; original completed card tender | Original card tender, actor | Durable `PosPostVoidCardPreparation` (`prepared`) under parent (eager from `PreparePostVoid`, or recreate after abandon) |
| `Pos::RecordPostVoidCardConfirmation` | Point of Sale | 6a | Yes | Yes (same auth replay) | Preparation row (txn lock before prep); parent must be approved for `prepared` → `recorded` | Preparation + terminal confirmation refs | Marks preparation `recorded`, or `recorded_orphan` when auth arrives after abandon (not consumable) |
| `Pos::AbandonPostVoidCardConfirmation` | Point of Sale | 6a | Yes | No | Prepared preparation | Preparation, actor | Abandons unused prepared confirmation |
| `Pos::FinalizeReturnFinancials` | Point of Sale | 6a | Yes | No | Return txn (caller may hold txn lock); pending return lines + related originals | Open return txn | Locks lines/originals, runs `ReassignReturnResiduals`, then `RecalculateTransaction`; used by tender prepare/add paths and open-return show |
...
| `Pos::PrepareCardRefund` | Point of Sale | 6d | Yes | No | Return txn + linked originals (`RefundLockOrder`) | Return txn, card tender type, amount, optional original / exception; optional `replaces_pos_tender` for recon replacement | Durable `PosCardRefundPreparation` (`prepared`); replacement path excludes the recon tender from capacity; plan snapshot + fingerprint; commercially locks txn |
| `Pos::AbandonCardRefundPreparation` | Point of Sale | 6d | Yes | No | Transaction + preparation | Outstanding preparation, actor | `abandoned` — TTL never auto-abandons |
| `Pos::ValidateCompletionReadiness` | Point of Sale | 6d | Yes (locks) | No | Transaction, pending lines, unresolved tenders, related originals | Open txn + actor | Non-posting readiness: recalc blockers, departments, sale eligibility, linked returns/refunds, allocation policy, tender settlement; used by complete and recon acceptance |
| `Pos::AddCardRefundTender` | Point of Sale | 6d | Yes | Yes (prep + auth) | Txn then preparation then linked originals | Preparation UUID + auth/terminal only | `recorded_tender` (open) or `recorded_orphan` (closed/abandoned); same auth = replay; different auth = conflict; config drift → recon reasons; store late-auth recovery uses this path |
| `Pos::ResolveCardRefundTenderReconciliation` | Point of Sale | 6d | Yes | No | Txn + preparation + tender | Preparation/tender, outcome (`externally_voided` / `validated_and_accepted` / `replaced`), reason; acceptance needs exception approval | Clears recon or voids tender; `pos.card_refund.reconcile` + `pos.tender.card_void` for voids; `validated_and_accepted` runs `ValidateCompletionReadiness` before clear; financial-exception path sets `resolution_pos_approval_id` |
| `Pos::ResolveCardRefundOrphan` | Point of Sale | 6d | Yes | No | Preparation | Orphan prep, `external_void_confirmed` or `accepted_financial_exception`, reason, refs | Resolves orphan; financial exception records `PosApproval` on `resolution_pos_approval_id` |
...
- **Card refund preparation.** Durable `pos_card_refund_preparations` bind terminal authorization to a server-owned plan. While `prepared`, the transaction is not commercially editable and cannot complete/cancel/suspend — `expires_at` is staleness/UI only. Recording accepts preparation id + auth only; open txn → authorized tender; closed or abandoned prep → `recorded_orphan` (not a tender on immutable history). Destination exception approval may use prepare-time `pos_approval_id`; financial-exception resolution uses `resolution_pos_approval_id`. Store late-auth recovery is on the orphan queue (`record_authorization`) and calls `AddCardRefundTender`. `validated_and_accepted` runs `ValidateCompletionReadiness` so acceptance can be followed immediately by completion.
