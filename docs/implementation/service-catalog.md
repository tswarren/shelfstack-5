...
| `Pos::AddCardRefundTender` | Point of Sale | 6d | Yes | Yes (prep + auth) | Txn then preparation then linked originals | Preparation UUID + auth/terminal only | `recorded_tender` (open) or `recorded_orphan` (closed); same auth = replay; different auth = conflict; config drift → recon reasons |
| `Pos::ResolveCardRefundTenderReconciliation` | Point of Sale | 6d | Yes | No | Txn + preparation + tender | Preparation/tender, outcome (`externally_voided` / `validated_and_accepted` / `replaced`), reason; acceptance needs exception approval | Clears recon or voids tender; `pos.card_refund.reconcile` + `pos.tender.card_void` for voids; acceptance revalidates allocation |
| `Pos::ResolveCardRefundOrphan` | Point of Sale | 6d | Yes | No | Preparation | Orphan prep, `external_void_confirmed` or `accepted_financial_exception`, reason, refs | Resolves orphan; financial exception records `PosApproval` |
...
- **Card refund preparation.** Durable `pos_card_refund_preparations` bind terminal authorization to a server-owned plan. While `prepared`, the transaction is not commercially editable and cannot complete/cancel/suspend — `expires_at` is staleness/UI only. Recording accepts preparation id + auth only; open txn → authorized tender; closed txn → `recorded_orphan` (not a tender on immutable history). Exception approval is created once at prepare and reused. Unresolved orphans appear in the store orphan queue.
