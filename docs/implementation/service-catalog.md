...
| `Pos::AddCashTender` | Point of Sale | 4c/6a | Yes | No | Transaction (`lock`); finalizes linked-return residuals before balance check | Transaction, cash Tender Type, amount tendered, actor | `pending` `PosTender` capped at finalized balance due |
| `Pos::ValidateTenderReferences` | Point of Sale | 6a | No | Yes | None | Tender Type + reference_1/2 (or authorization_code/terminal_reference) | Normalized refs; enforces TenderType requirement/mask rules |
| `Pos::ValidateTenderReferences` | Point of Sale | 6a | No | No | None | Tender Type + submitted refs | Maps TenderType reference slots to `authorization_code` / `terminal_reference` |
| `Pos::CardRecordingIdempotency` | Point of Sale | 6a | No | No | Caller-held txn + tender lock | Request UUID + payload fields | `:proceed` / identical `:replay` / payload `:conflict` (internal helper) |
| `Pos::AddCardTender` | Point of Sale | 4c/6a | Yes | No | Transaction (`lock`); keyed replay before current TenderType ref validation | Transaction, card Tender Type, amount, refs, request UUID, actor | `authorized` when attachable; attach failure → `void_required` under same key; conflicting payload → explicit error (no new tender) |
| `Pos::RecordVoidedCardTender` | Point of Sale | 6a | Yes | No | Transaction + tender (`lock`); requires `pos.tender.card_void` | `void_required` `PosTender`, external-void confirmation, actor | Transitions tender to `voided`; idempotent if already voided; independent of txn open / tender-type active flags |
| `Pos::VoidCardTender` | Point of Sale | 6a | Yes | No | Transaction + tender (`lock`); requires `pos.tender.card_void` | Authorized card tender, actor, external-void confirmation | Marks attached tender `voided` (excluded from settlement) |
| `Pos::RetainVoidRequiredCardTender` | Point of Sale | 6a | Yes | No | Caller-held txn lock | Internal helper (not a public workflow) | Creates or returns tender by caller-supplied `recording_idempotency_key` |
| `Pos::RemoveTender` | Point of Sale | 4c | Yes | No | Tender (`lock`) | Tender, actor, optional reason | `pending` → `removed`; authorized card → `VoidCardTender`; restores `PosTransaction#editable?` |
...
| Service | Domain owner | Introduced | Transactional? | Idempotent? | Locks | Input | Result |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `Pos::AddLinkedReturnLine` | Point of Sale | 4e | Yes | No | Transaction (`lock`) + original line (`lock`); requires `pos.return.create` | Open Transaction, completed original sale line (not `stored_value`), quantity, Return Reason, disposition, actor | Pending `direction: return` line copying original commercial/cost snapshots; proportional historical Discount allocations (INV-RET-004); tax components reversed from original snapshots via `Pos::RecalculateTransaction`; rejects stored-value lines; never mutates the original sale line |
| `Pos::AddUnlinkedReturnLine` | Point of Sale | 11B | Yes | No | Transaction (`lock`); `pos.return.create`; `no_receipt` also `pos.return.no_receipt` + `maximum_no_receipt_return_cents` (`AuthorizeAction` / `no_receipt_return`) inside the same transaction after lock | Open Transaction; source `external_receipt`/`gift_receipt`/`no_receipt`; reason; disposition; explicit `unit_price_cents`; tax basis (`current_configured_rules` / `external_receipt_tax` / `no_tax_refund`) + optional explicit tax; cost confirmation for inventory-affecting quantity-tracked dispositions (MAC → department estimate → block); product variant (quantity/none) **or** open-ring department/description; optional approver | Pending return line with durable tax/cost basis snapshots; tax via configured rules, explicit external amount, or no-tax; cost quality `estimated` when inbound; rejects individually tracked variants; inventory via `PostCustomerReturn` on product complete |
| `Pos::StartUnlinkedReturn` | Point of Sale | 11B | Yes | No | Session (`lock`); delegates to `AddUnlinkedReturnLine` | Open Session + unlinked return attrs | Opens/reuses Transaction and adds first unlinked return line atomically (Ready first-valid-work); failure leaves no empty Transaction |
| `Pos::RecordNoSale` | Point of Sale | 11 | Yes | Yes (idempotency key) | Open cash-enabled Session (`lock`); `pos.no_sale.create` | Open Session, actor, reason, idempotency key | Immutable `PosNoSaleEvent` + audit; no Cash Movement; no drawer kick |
| `Pos::ProductLookupResults` | Point of Sale | 11B | No | No | — | Organization, store, query | Bounded POS-specific Product/Variant groups with price, availability, tracking mode, eligibility blockers, optional inventory-unit context; shared by overlay and `_scan_resolution` |
| `Pos::LineActions` | Point of Sale | 11 | No | No | — | User, store, line | Presenter for `discount` / `price.override` / `tax_category.override` / `line.remove` availability |
...
| `Pos::ValidateCompletionReadiness` | Point of Sale | 6d | Yes (locks) | No | Transaction, pending lines, unresolved tenders, related originals | Open txn + actor | Non-posting readiness: recalc blockers, departments, sale eligibility, linked returns/refunds, allocation policy, tender settlement |
| `Pos::AddCardRefundTender` | Point of Sale | 6d | Yes | No | Txn + linked originals; keyed replay before current TenderType ref validation | Return txn, card tender type, amount, new refs, request UUID, optional original / exception | Direct `authorized` refund; attach failure → `void_required` under same key; conflicting payload → explicit error |
...
| `Pos::AddStoredValueTender` | Point of Sale | 6d | Yes | No | Transaction lock, then finalize linked-return residuals, then SV account lock | Open txn, SV tender type, account, amount, actor | Pending redeem tender against finalized balance |
...
- **Refund allocation.** `Pos::RefundAllocationPolicy` validates the whole current-transaction refund plan (SV-first). Completed refunds reduce capacity; unresolved refunds in other transactions are in-flight blockers. Non-original destinations require approver permission `pos.return.refund_exception.approve`, or `pos.return.refund_exception.approve_self` for recorded self-approval with PIN re-auth (requester holds the destination tender permission). Completion revalidates the plan under locks; `Pos::RemoveTender` locks original sale tenders when clearing linked refunds.
...
| `Catalog::Enrichment::BuildNormalizedResult` | Catalog and Products | 8b | No | Yes | None | Provider-mapped plain-hash fields (title, creators, publication date, list price, etc.) | Immutable, deep-frozen `Catalog::Enrichment::NormalizedResult`; normalizes creator roles (unknown/missing → `contributor` + warning) and list price (BigDecimal, half-up cents, uppercase-or-null currency) |
| `Catalog::Enrichment::NormalizeCreators` / `NormalizeMoney` | Catalog and Products | 8b | No | Yes | None | Raw provider creator/money fields | Helpers used by `BuildNormalizedResult`; independently unit-tested |
| `Catalog::Providers::ParseProviderDate` | Catalog and Products | 8b (rev.) | No | Yes | None | Provider date string | Exact `Date` for `YYYY-MM-DD…` only; `nil` for year/month-only or unrecognized text |
| `Catalog::Providers::NetHttpTransport` | Catalog and Products | 8b | No | Yes | None | URL (HTTPS only, fixed adapter base constants), headers | One bounded HTTPS attempt (connect/read timeout, capped body size); raises typed `ConnectionError`/`TimeoutError`/`ResponseTooLargeError` — never retries itself |
| `Catalog::Providers::RetryPolicy` | Catalog and Products | 8b | No | Yes | None | Injected transport + request | Drives at most one retry for connection/timeout errors or HTTP 502/503/504 against whichever transport (real or fake) it is given; HTTP 429 is never retried here |
| `Catalog::Providers::Isbndb` / `GoogleBooks` | Catalog and Products | 8b | No | Yes | None | Canonical ISBN-13, injected transport/API key | `Catalog::Providers::AdapterResult`: `success(NormalizedResult)` or `failure(code, message, metadata)`; Google Books keeps only exact canonical-ISBN matches (duplicate exact hits → `ambiguous_result`); authorization-free and independently testable |
| `Catalog::LookupExternalMetadata` | Catalog and Products | 8b | No | Yes | None | Actor, store, identifier, provider (`:isbndb`/`:google_books`) | Requires `catalog.lookup_external`; only a valid ISBN-13/Bookland EAN-13 (978/979) proceeds to a provider call (else `unsupported_identifier`, zero transport calls); unknown provider raises `ArgumentError`; creates no Product/Variant/Creator/ProductCreator/CatalogEnrichmentEvent rows |
| `Catalog::MapProductFormat` | Catalog and Products | 8b | No | Yes | None | Organization, provider-format string | `FormatSuggestion` (existing active `ProductFormat` or nil) via exact code/name match or a small explicit token map; several matches → nil + warning; never creates/writes a `ProductFormat` |
| `Catalog::FindExistingProduct` | Catalog and Products | 8c | No | Yes | None | Organization, identifier | Org-scoped local find via `Catalog::Lookup`; returns `FindResult` (product/products/match_kind/canonical); no provider call |
| `Catalog::PreviewProductImport` | Catalog and Products | 8c | No | Yes | None | Actor, store, organization, identifier, provider, optional transport | Requires `catalog.lookup_external`; existing → `:existing` (zero provider calls); else lookup → preview DTO (proposed attrs, format/list-price/creator suggestions, sale-eligibility gaps); **no DB writes** / no enrichment event |
| `Catalog::ProductImportPreviewToken` | Catalog and Products | 8c | No | Yes | None | Organization, actor, normalized result, warnings | Issues/verifies a 30-minute signed MessageVerifier payload (`catalog.product_import_preview`) binding org + actor + immutable provider provenance and list-price snapshot; accept must `verify!` before create |
| `Catalog::CreateFromEnrichment` | Catalog and Products | 8c | Yes | No* | Sequence + product uniqueness | Accepted preview fields, creator resolutions, **caller-verified** provider provenance | Requires `catalog.create_from_enrichment` **and** `catalog.product.create`; txn creates Product + Variant + Creators + one `catalog_enrichment_events` (`action: create`); provider call must already be done; list price never becomes variant regular price; sellable forced false |
...
| `StoredValue::BalanceAfterEntry` | Stored Value | 11.1 | No | Yes | None | Ledger entry | Historical balance through entry (ledger order) |
| `StoredValue::ActivitySlipFacts` | Stored Value | 11.1 | No | Yes | None | Entry, store, reprint flag | Masked Activity Slip presentation facts for issued/reloaded/refunded |
| `StoredValue::CreditVoucherFacts` | Stored Value | 11.1 | No | Yes | None | Account, store, reprint flag | Credit Voucher facts; raises when account suspended |
| `StoredValue::AccountBarcode` | Stored Value | 11.1 | No | Yes | None | Canonical account number | EAN-13 SVG (or nil + error); payload is the account number |
...
- Activity Slip balances are historical; Credit Voucher balances are current at print time.
- Immediate context may also plant in-scope SV entry/account ids after completion.
