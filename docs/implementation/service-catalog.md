...
| `Pos::AddCardTender` | Point of Sale | 4c/6a | Yes | No | Transaction (`lock`); finalizes linked-return residuals before balance check | Transaction, card Tender Type, amount, refs, actor | `authorized` `PosTender` when attachable; after refs validate, attach failure persists idempotent `void_required` tender (`requires_void_confirmation?`) |
| `Pos::RetainVoidRequiredCardTender` | Point of Sale | 6a | Yes | No | Caller-held txn lock | Transaction, card Tender Type, amount, direction, refs, actor, reason | Creates or returns existing `void_required`/`voided` tender by `recording_idempotency_key` |
...
| `Pos::VoidCardTender` | Point of Sale | 6a | Yes | No | Transaction + tender (`lock`) | Authorized card tender, actor, external-void confirmation | Marks tender `voided` (excluded from settlement); requires `pos.tender.card_void` |
...
| `Pos::AddCardRefundTender` | Point of Sale | 6d | Yes | No | Txn + linked originals | Return txn, card tender type, amount, new refs, optional original / exception | Direct `authorized` refund tender; after refs validate, attach failure persists idempotent `void_required` tender |
...
- **Standalone card recording.** ShelfStack records operator-confirmed terminal activity only ([ADR-0016](../adr/0016-treat-standalone-credit-card-activity.md); internal completion still under ADR-0009). Partial card tenders are allowed within remaining balances. After references validate, unattachable terminal activity is persisted as `void_required` (excluded from settlement; blocks complete/suspend/cancel) and resolved to `voided` via `RecordVoidedCardTender` (`pos.tender.card_void`). Post-void uses Policy A: approve first (`ApprovePostVoid`), reverse cards on the terminal, enter confirmations, durable audit events, then `PostVoidTransaction`. No prep/orphan/recon tables. Chargebacks and processor settlement remain Phase 7.
