...
- Gate 8c Slice 1 (services): `FindExistingProduct` / `PreviewProductImport` / `CreateFromEnrichment`; permission `catalog.create_from_enrichment` (persist still also needs `catalog.product.create`). Preview never writes; create writes one enrichment event in the same transaction.
