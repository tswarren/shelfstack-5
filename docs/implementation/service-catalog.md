...
| `Pos::AddCardTender` | Point of Sale | 4c/6a | Yes | No | Transaction (`lock`); finalizes linked-return residuals before balance check | Transaction, card Tender Type, amount, refs, actor | `authorized` `PosTender` when `0 < amount <= remaining received balance` (unless over-tender allowed); otherwise `amount_mismatch?` for record-and-void |
| `Pos::RecordVoidedCardTender` | Point of Sale | 6a | Yes | No | Transaction (`lock`) | Transaction, card Tender Type, attempted amount, refs, external-void confirmation, actor | Creates already-`voided` tender retaining refs/amount after attach failure |
...
| `Pos::AddCardRefundTender` | Point of Sale | 6d | Yes | No | Txn + linked originals | Return txn, card tender type, amount, new refs, optional original / exception | Direct `authorized` refund tender; `amount_mismatch?` when amount not attachable (record-and-void via `RecordVoidedCardTender`) |
...
- **Standalone card recording.** ShelfStack records operator-confirmed terminal activity only (ADR-0009). Partial card tenders are allowed within remaining balances. Every entered authorization that cannot be attached becomes a `voided` tender via `RecordVoidedCardTender` after external-void confirmation. Post-void uses Policy A: approve first (`ApprovePostVoid`), reverse cards on the terminal, enter confirmations, durable audit events, then `PostVoidTransaction`. No prep/orphan/recon tables. Chargebacks and processor settlement remain Phase 7.
