...
| `Pos::PrepareCardRefund` | Point of Sale | 6d | Yes | No | Return txn + linked original txns/tenders (`RefundLockOrder`) | Return txn, card tender type, amount, optional original / exception | Preflight only — validates plan before terminal use |
| `Pos::AddCardRefundTender` | Point of Sale | 6d | Yes | No | Return txn + linked originals (`RefundLockOrder`) | Return txn, amount, auth code (external already approved), optional original / exception | Always retains `authorized` refund tender; `requires_reconciliation` when post-auth validation fails; completion blocked until resolved |
