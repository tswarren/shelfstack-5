...
| OD-006 | Customer identity for Phase 5 requests | superseded | Phase 5 / 9 | Product Requests / Customers | Opaque `customer_reference` interim superseded by [ADR-0017](../adr/0017-customer-domain-and-namespace-22.md) / Phase 9 `customers` + `customer_id`. Rich CRM remains deferred (DWR-036). |
...
| OD-011 | Identifier generation sequence ownership | accepted | Phase 2 | Catalog | Installation-singleton `identifier_sequences` (namespaces `21`/`22`/`27`/`28`/`29`); INV-ORG-001. Issue [#14](https://github.com/tswarren/shelfstack-5/issues/14); `22` via ADR-0017 |
